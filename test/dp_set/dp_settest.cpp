...
template <typename T>
inline bool check_set( rp_set<T> const& ls, std::initializer_list<typename value_p<T>::t> const& ts )
{
	rp_set<T> l;
...
void dp_settest::test01()
{
	...
	// copy constructor
	rp_set<test*>		p0( lr );
	const rp_set<test*>	c0( lr );
	lr.insert( &t1 );
	rp_set<test*>		p1( lr );
	const rp_set<test*>	c1( lr );
	lr.insert( &t2 );
	lr.insert( &t3 );
	lr.insert( &t4 );
	lr.insert( &t5 );
	rp_set<test*>		p5( lr );
	const rp_set<test*>	c5( lr );
	...
	// move assignment operator
	{
		...
		lr = p1;
		lx = rp_set<test*>( { &t5, &t3, &t2 } );		// move assignment by r-value
	...
	{
		rp_set<test*>   lx = p5;
		rp_set<test*>::iterator	i = lx.find( &t4 );

		rp_set<test*>   x3( lx.begin(), i );
		AssertTrue( check_set( x3, { &t1, &t2, &t3 } ));
		rp_set<test*>   x0( i, i );
		AssertTrue( x0.empty());
		rp_set<test*>   x1( i, lx.end());
...
void dp_settest::test03()
{
	...
	// default constructor
	rp_set<const char[]>		lr;
	AssertTrue( check_set( lr, { } ));

	// copy constructor
	rp_set<const char[]>		p0( lr );
	const rp_set<const char[]>	c0( lr );
	lr.insert( s1 );
	rp_set<const char[]>		p1( lr );
	const rp_set<const char[]>	c1( lr );
	lr.insert( s2 );
	lr.insert( s3 );
	lr.insert( s4 );
	lr.insert( s5 );
	rp_set<const char[]>		p5( lr );
	const rp_set<const char[]>	c5( lr );
	...
	// move constructor
	{
		rp_set<const char[]>   lx = lr;
		rp_set<const char[]>   ly( std::move( lx ));
	...
	// move assignment operator
	{
		...
		lr = p1;
		lx = rp_set<const char[]>( { s5, s3, s2 } );		// move assignment by r-value
	...
	// iterator constructor
	{
		rp_set<const char[]>   lx = p5;
		rp_set<const char[]>::iterator	i = lx.find( s4 );

		rp_set<const char[]>   x3( lx.begin(), i );
		AssertTrue( check_set( x3, { s1, s2, s3 } ));
		rp_set<const char[]>   x0( i, i );
		AssertTrue( x0.empty());
		rp_set<const char[]>   x1( i, lx.end());
		AssertTrue( check_set( x1, { s4, s5 } ));
	}
	// initializer constructor/assignment operator
	{
		rp_set<const char[]>   lx = p5;
		rp_set<const char[]>   x0 { };
		rp_set<const char[]>   x1 { s1 };
		rp_set<const char[]>   x5 { s1, s2, s3, s4, s5 };
...
void dp_settest::test05()
{
	...
	// move assignment operator
	{
		...
		lr = p1;
		lx = rp_set<test[]>( { v5, v3, v2 } );		// move assignment by r-value
	...
	{
		rp_set<test[]>   lx = p5;
		rp_set<test[]>::iterator	i = lx.find( v4 );
...
void dp_settest::test07()
{
	...
	// copy constructor
	rp_set<test[3]>		p0( lr );
	const rp_set<test[3]>	c0( lr );
	lr.insert( w1 );
	rp_set<test[3]>		p1( lr );
	const rp_set<test[3]>	c1( lr );
	lr.insert( w2 );
	lr.insert( w3 );
	lr.insert( w4 );
	lr.insert( w5 );
	rp_set<test[3]>		p5( lr );
	const rp_set<test[3]>	c5( lr );
	...
	// move assignment operator
	{
		...
		lr = p1;
		lx = rp_set<test[3]>( { w5, w3, w2 } );		// move assignment by r-value
	...
	{
		rp_set<test[3]>   lx = p5;
		rp_set<test[3]>::iterator	i = lx.find( w4 );

		rp_set<test[3]>   x3( lx.begin(), i );
		AssertTrue( check_set( x3, { w1, w2, w3 } ));
		rp_set<test[3]>   x0( i, i );
		AssertTrue( x0.empty());
		rp_set<test[3]>   x1( i, lx.end());
	...
	// initializer constructor/assignment operator
	{
		...
		rp_set<test[3]>   x0 { };
		rp_set<test[3]>   x1 { w1 };
		rp_set<test[3]>   x5 { w1, w2, w3, w4, w5 };
...
void dp_settest::test0A()
{
	...
	rp_set<test*>      s = { &t1, &t2, &t5, &t8, &t4, &t2, &t3, &t6, &t3, &t4, &t5, &t4, &t4, &t1, &t2 };
	...
	rp_set<test*>			l1, r1, l2, r2, l3, r3, l4, r4, l5, r5;
	rp_set<test*>::iterator	b, e;
...
void dp_settest::test0C()
{
	...
	rp_set<const char[]>      s = { s1, s2, s5, s8, s4, s2, s3, s6, s3, s4, s5, s4, s4, s1, s2 };
	...
	rp_set<const char[]>			l1, r1, l2, r2, l3, r3, l4, r4, l5, r5;
	rp_set<const char[]>::iterator	b, e;
...
void dp_settest::test0E()
{
	...
	rp_set<test[]>      s = { v1, v2, v5, v8, v4, v2, v3, v6, v3, v4, v5, v4, v4, v1, v2 };
...
void dp_settest::test10()
{
	...
	rp_set<test[3]>      s = { w1, w2, w5, w8, w4, w2, w3, w6, w3, w4, w5, w4, w4, w1, w2 };
	...
	rp_set<test[3]>			l1, r1, l2, r2, l3, r3, l4, r4, l5, r5;
	rp_set<test[3]>::iterator	b, e;
...
void dp_settest::test13()
{
	...
	rp_set<test*>	m1 = { &t1, &t2 };
	rp_set<test*>	m2;
	rp_set<test*>	m3 = { &t2, &t3, &t5 };

	// emplace
	{
		typedef std::pair<typename rp_set<test*>::iterator,bool>	pair;
		typedef typename rp_set<test*>::iterator			iterator;
	...
	// erase
	{
		rp_set<test*>	p { &t1, &t2, &t5, &t8, &t4, &t3, nullptr };
		rp_set<test*>	p1( p );
...
void dp_settest::test15()
{
	...
	rp_set<const char[]>	m1 = { s1, s2 };
	rp_set<const char[]>	m2;
	rp_set<const char[]>	m3 = { s2, s3, s5 };

	// emplace
	{
		typedef std::pair<typename rp_set<const char[]>::iterator,bool>	pair;
		typedef typename rp_set<const char[]>::iterator			iterator;

		rp_set<const char[]>	pm;
		rp_set<const char[]>	pi;
		pair		r;
		iterator	j = pi.begin();
	...
	{
		rp_set<const char[]>	pm;
		rp_set<const char[]>	il;
		rp_set<const char[]>	it;
	...
	// erase
	{
		rp_set<const char[]>	p { s1, s2, s5, s8, s4, s3, nullptr };
		rp_set<const char[]>	p1( p );
		...
		rp_set<const char[]>	q1( p );
		rp_set<const char[]>	q2( p );
		rp_set<const char[]>	q3( p );
		rp_set<const char[]>	q4( p );
...
void dp_settest::test17()
{
	...
	rp_set<test[]>	m1 = { v1, v2 };
	rp_set<test[]>	m2;
	rp_set<test[]>	m3 = { v2, v3, v5 };
	...
	// erase
	{
		rp_set<test[]>	p { v1, v2, v5, v8, v4, v3, nullptr };
		rp_set<test[]>	p1( p );
...
void dp_settest::test19()
{
	...
	rp_set<test[3]>	m1 = { w1, w2 };
	rp_set<test[3]>	m2;
	rp_set<test[3]>	m3 = { w2, w3, w5 };

	// emplace
	{
		typedef std::pair<typename rp_set<test[3]>::iterator,bool>	pair;
		typedef typename rp_set<test[3]>::iterator			iterator;
	...
	// erase
	{
		rp_set<test[3]>	p { w1, w2, w5, w8, w4, w3, nullptr };
		rp_set<test[3]>	p1( p );
...
void dp_settest::test1C()
{
	...
	rp_set<test*>	pm = { &t1, &t2, &t3, &t5 }; 
	{
		rp_set<test*>::iterator		i;
		rp_set<test*>::const_iterator	j, k;
		unsigned			c;

		for ( i = pm.begin(), c = 0; i != pm.end(); ++i, ++c ) {
			if ( c == 1 ) k = rp_set<test*>::iterator( i );
			if ( c == 2 ) {
	...
	{	
		rp_set<test*>::reverse_iterator		i;
		rp_set<test*>::const_reverse_iterator	j, k;
		unsigned				c;

		for ( i = pm.rbegin(), c = 0; i != pm.rend(); ++i, ++c ) {
			if ( c == 2 ) k = rp_set<test*>::reverse_iterator( i );
			if ( c == 3 ) {
		...
		AssertEqual( c, 4 );
		AssertEqual( static_cast<rp_set<test*>::const_reverse_iterator>( pm.find( &t2 )), j );
		AssertTrue( k != j );
		AssertTrue( ++k == j );
	}
}
...
void dp_settest::test1E()
{
	...
	rp_set<const char[]>	pm = { s1, s2, s3, s5 }; 
	{
		rp_set<const char[]>::iterator		i;
		rp_set<const char[]>::const_iterator	j, k;
		unsigned			c;

		for ( i = pm.begin(), c = 0; i != pm.end(); ++i, ++c ) {
			if ( c == 1 ) k = rp_set<const char[]>::iterator( i );
			if ( c == 2 ) {
				AssertNotEqual( i, pm.end());
				AssertEqual( *i, s3 );
	...
	{	
		rp_set<const char[]>::reverse_iterator		i;
		rp_set<const char[]>::const_reverse_iterator	j, k;
		unsigned				c;

		for ( i = pm.rbegin(), c = 0; i != pm.rend(); ++i, ++c ) {
			if ( c == 2 ) k = rp_set<const char[]>::reverse_iterator( i );
			if ( c == 3 ) {
				AssertNotEqual( i, pm.rend());
	  			AssertEqual( *i, s1 );
		...
		AssertEqual( i, pm.rend());
		AssertEqual( c, 4 );
		AssertEqual( static_cast<rp_set<const char[]>::const_reverse_iterator>( pm.find( s2 )), j );
		AssertTrue( k != j );
		AssertTrue( ++k == j );
	}
}
...
void dp_settest::test20()
{
	...
	{	
		...
		AssertEqual( i, pm.rend());
		AssertEqual( c, 4 );
		AssertEqual( static_cast<rp_set<test[]>::const_reverse_iterator>( pm.find( v2 )), j );
		AssertTrue( k != j );
		AssertTrue( ++k == j );
	}
}
...
void dp_settest::test22()
{
	...
	rp_set<test[3]>	pm = { w1, w2, w3, w5 }; 
	{
		rp_set<test[3]>::iterator		i;
		rp_set<test[3]>::const_iterator	j, k;
		unsigned			c;

		for ( i = pm.begin(), c = 0; i != pm.end(); ++i, ++c ) {
			if ( c == 1 ) k = rp_set<test[3]>::iterator( i );
			if ( c == 2 ) {
				AssertNotEqual( i, pm.end());
				AssertEqual( *i, w3 );
	...
	{	
		rp_set<test[3]>::reverse_iterator		i;
		rp_set<test[3]>::const_reverse_iterator	j, k;
		unsigned				c;

		for ( i = pm.rbegin(), c = 0; i != pm.rend(); ++i, ++c ) {
			if ( c == 2 ) k = rp_set<test[3]>::reverse_iterator( i );
			if ( c == 3 ) {
				AssertNotEqual( i, pm.rend());
	  			AssertEqual( *i, w1 );
		...
		AssertEqual( i, pm.rend());
		AssertEqual( c, 4 );
		AssertEqual( static_cast<rp_set<test[3]>::const_reverse_iterator>( pm.find( w2 )), j );
		AssertTrue( k != j );
		AssertTrue( ++k == j );
	}
}
...
void dp_settest::test25()
{
	...
	rp_set<test*>      r1;
	rp_set<test*>      r2 = { &t1, &t2, &t3, &t5 };
	rp_set<test*>      r3 = { &t4, &t5 };
...
void dp_settest::test27()
{
	...
	rp_set<const char[]>      r1;
	rp_set<const char[]>      r2 = { s1, s2, s3, s5 };
	rp_set<const char[]>      r3 = { s4, s5 };
...
void dp_settest::test29()
{
	...
	rp_set<test[]>      r1;
	rp_set<test[]>      r2 = { v1, v2, v3, v5 };
	rp_set<test[]>      r3 = { v4, v5 };
...
void dp_settest::test2B()
{
	...
	rp_set<test[3]>      r1;
	rp_set<test[3]>      r2 = { w1, w2, w3, w5 };
	rp_set<test[3]>      r3 = { w4, w5 };
...
void dp_settest::test2E()
{
	...
	rp_set<test*>      r1 = { &t1, &t2 };
	rp_set<test*>      r2 = { &t1, &t2, &t3, &t5 };
	rp_set<test*>      r3 = { &t1, &t3, &t2 };
	rp_set<test*>      r4 = { &t1, &t2, &t3, &t5 };
	rp_set<test*>      r5;
...
void dp_settest::test30()
{
	...
	rp_set<const char[]>      r1 = { s1, s2 };
	rp_set<const char[]>      r2 = { s1, s2, s3, s5 };
	rp_set<const char[]>      r3 = { s1, s3, s2 };
	rp_set<const char[]>      r4 = { s1, s2, s3, s5 };
	rp_set<const char[]>      r5;
...
void dp_settest::test32()
{
	...
	rp_set<test[]>      r1 = { v1, v2 };
	rp_set<test[]>      r2 = { v1, v2, v3, v5 };
	rp_set<test[]>      r3 = { v1, v3, v2 };
	rp_set<test[]>      r4 = { v1, v2, v3, v5 };
	rp_set<test[]>      r5;
...
void dp_settest::test34()
{
	...
	rp_set<test[3]>      r1 = { w1, w2 };
	rp_set<test[3]>      r2 = { w1, w2, w3, w5 };
	rp_set<test[3]>      r3 = { w1, w3, w2 };
	rp_set<test[3]>      r4 = { w1, w2, w3, w5 };
	rp_set<test[3]>      r5;
