...
{       
	...
	deep_ptr( dp_base<T*> const& p ) : dp_base<T*>( dp_dup<T*>()( p.get())) { }
	deep_ptr( dp_base<T*>&& p )      : dp_base<T*>( p.release() ) { }
	deep_ptr& operator=( dp_base<T*> const& p ) { this->copy( p.get()); return *this; }
	deep_ptr& operator=( dp_base<T*>&& p )	   { this->move( p.release()); return *this; }   
	...
	operator const const_ptr&() const	{ return const_cast<const const_ptr&>( this->m_p ); }
...
{       
	...
	dref_ptr( deep_ptr<T*> const& p ) : dp_base<T*>( const_cast<value_ptr>( p.get())) { }
	dref_ptr& operator=( deep_ptr<T*> const& p ) { this->set( p.get()); return *this; }
...
{       
	...
	dref_ptr( deep_ptr<T[]> const& p ) : dp_base<T[]>( p.get()) { }
	dref_ptr& operator=( deep_ptr<T[]> const& p ) { this->set( p.get()); return *this; }
...
{       
	...
	dref_ptr( deep_ptr<T[N]> const& p ) : dp_base<T[N]>( p.get()) { }
	dref_ptr& operator=( deep_ptr<T[N]> const& p ) { this->set( p.get()); return *this; }
